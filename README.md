...
- **Random Forest for the model** — handles non-linear relationships, needs no feature scaling, and exposes feature importance directly, fitting the goal of understanding salary drivers
...
> **Validation:** the model is evaluated on a held-out 20% test set (data unseen during training), and a fixed `random_state` makes the whole pipeline reproducible.

---

## 📊 Power BI Dashboard

The interactive dashboard explores these results dynamically across three pages — Salary Overview, Job Market Trends, and ML Insights.
...
> The dashboard is built on the cleaned dataset plus two CSVs exported from the Python model (feature importance and actual-vs-predicted). Computation happens in Python; Power BI is the presentation layer.
...
**Objective:** predict salary from experience, education, job attributes, and work mode — and identify which factors drive it most.
...
> Note: years of experience and seniority are correlated, so the model splits importance across the two — they are best read together as one "experience signal" of ~65%, rather than as two separate numbers.
...
- Add unit tests on the cleaning functions for production robustness
